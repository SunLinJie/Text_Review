...
class AI_gate():
    ...
    def make_app(self, myaigroup):
        ...
        def health():
            dic={}
            try:
                with open('./testimg/1.jpg', 'rb') as f:
                    now = time.time()
                    data = f.read()
                    f.close
        ...
        def metrics():
            ...
            g = "build_time {}".format(os.getenv("build_nu"))
            return Response("{}{}{}{}{}{}{}".format(a,b,c,d,e,f,g), mimetype="text/plain")
        ...
        def get_tasks():
            ...
            try:
                ...
                try:
                    ...
                    image_data_base64 = urllib.request.unquote((request.form.get('imagedata')))
                    if (len(image_data_base64) > 20):
                        # print("get b64:"+image_data_base64)
                ...
                ret = make_error(self.models_str, 'imgdata', 'fail fot geting image from data')
                ret.update({"pro_ver": "{}".format(self.pro_ver)})
                self.disulog.error(ret)
                return Response(json.dumps(ret), mimetype='application/json')
            ...
            try:
                # 根据imgurl获得图片
                if image_data == None:
                    image_data = urllib.request.urlopen(req_imgurl).read()
            except (Exception) as e:
                ret = make_error(self.models_str, 'imgurl', 'fail for geting image from url：{}'.format(req_imgurl))
                ret.update({"pro_ver": "{}".format(self.pro_ver)})
                self.disulog.error(ret)
                return Response(json.dumps(ret), mimetype='application/json')
